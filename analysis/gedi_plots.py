from connsql import logs_db_conn, props_db_conn
...
def update_rainfall_dash(bu_id:int, window:DateWindow, config:dict, real_time:bool):
    ...
    for index,s in df_rain_sensors.iterrows():
        try:
            ...
            # prd = processed_rainfall_data
            
            processed_rainfall_data["all"].to_csv(data_dir+s.site_name+" "+s.sensor_name+".csv") 
...
def get_plots_rain(rain_data, sensor) -> go.Figure:
    ...
    try:
        fig.update_layout(
            title_text=f"{str(sensor.site_name.item())} - {str(sensor.sensor_name.item())} Cumulative Rainfall (mm)",
            legend=dict(orientation="h",yanchor="bottom",y=1.02,xanchor="right",x=1),
        )
    except AttributeError:
        fig.update_layout(
            title_text=f"{str(sensor.site_name)} - {str(sensor.sensor_name)} Cumulative Rainfall (mm)",
            legend=dict(orientation="h",yanchor="bottom",y=1.02,xanchor="right",x=1),
        )
...
def update_tilt_dash(bu_id:int, window:DateWindow, config:dict, real_time:bool):
    ...
    for index,s in df_tilt_sensors.iterrows():
        ...
        try:
            ...
            print(f"No vel figure available for {s.site_name.item()} - {s.sensor_name}")
            continue
...
def update_soms_dash(bu_id:int, window:DateWindow, config:dict, real_time:bool):
    ...
    for index,s in df_soms_sensors.iterrows():
        try:
            ...
            print(f"No data available for {s.site_name.item()} - {s.sensor_name}")
            continue

        df_list.append({"df": df_p, "name": s.sensor_name, "site": s.site_name.item()})
        n_soms+=1
...
def update_power_dash(bu_id:int, window:DateWindow, config:dict, real_time:bool):
    ...
    for index,s in df_voltage_sensors.iterrows():
        try:            
            ...
            print("No figure available for {site_name} - {sensor_name}".format(sensor_name=s.sensor_name.item(), 
                                                                               site_name=s.site_code.item()))
            continue 

        fig.add_trace(go.Scatter(
            mode="lines+markers",
            x=df_ps.log_datetime,
            y=df_ps.voltage, name=s.site_name.item(),
            )
        )
