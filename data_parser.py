...
def connsql(db_type="logs"):
    ...
    cred_file = '/etc/gedi/db_cred.yaml'
    ...
    if (db_type=="props"):
        ...
        logs_db_conn = MySQLdb.connect(user=conn["user"], password=conn["pass"],
                                  host=conn["host"], database=conn["schema"])
        logs_db_conn.cursor().execute("SET SESSION MAX_EXECUTION_TIME=2000")
...
def get_unparsed_messages(limit):
    ...
    SQL_QUERY = ("SELECT id, message FROM message_transactions "
        "WHERE "
        "feed_name IS NOT NULL AND "
        "parsed_datetime IS NULL "
        "AND error_parse_datetime IS NULL "
        "ORDER BY id DESC " 
        "LIMIT {limit}"
        )
    SQL_QUERY = SQL_QUERY.format(limit=limit)
...
def get_logger_props(msg, d_type, search_for):
    ...
    if (d_type=="tilt"):
        ...
        search_tag = "where r.id={logger_id}".format(logger_id=search_for)
    elif (d_type=="gkn_tilt"):
        query += "FROM gkn_tilt_sensors r "
        search_tag = "where r.id={logger_id}".format(logger_id=search_for)
    else:
...
def insert_power_log(msg):
    try:
        # gateway power log
        SQL_INSERT = "INSERT IGNORE INTO power_sensor_logs (log_datetime, voltage, power, power_sensor_id) values (%s, %s, %s, %s);"
        values = (msg["log_dt"], msg["msg"]["BV"], msg["msg"]["PW"], msg["msg"]["PI"])    
    except KeyError:
        # sensor power log
        SQL_INSERT = "INSERT IGNORE INTO power_sensor_logs (log_datetime, voltage, power_sensor_id) values (%s, %s, %s);"
        values = (msg["log_dt"], msg["msg"]["BV"], int(msg["power_props"]["sensor_id"]))
    sql_execute_commit(SQL_INSERT, values)

def insert_tilt_log(msg):
    SQL_INSERT = "INSERT IGNORE INTO position_sensor_logs (log_datetime, ax_x, ax_y, ax_z, position_sensor_id) values (%s, %s, %s, %s, %s);"
    values = (msg["log_dt"], float(msg["msg"]["AX"]), float(msg["msg"]["AY"]), float(msg["msg"]["AZ"]), int(msg["tilt_props"]["sensor_id"]))    
    # print(msg)
    sql_execute_commit(SQL_INSERT, values)
    
def insert_soil_log(msg):
    SQL_INSERT = "INSERT IGNORE INTO soil_moisture_sensor_logs (log_datetime, ratio, soil_moisture_sensor_id) values (%s, %s, %s);"
    values = (msg["log_dt"], msg["msg"]["SO"], int(msg["soil_props"]["sensor_id"]))
    sql_execute_commit(SQL_INSERT, values)
    
def insert_rain_log(msg):
    SQL_INSERT = "INSERT IGNORE INTO rain_gauge_sensor_logs (log_datetime, num_ticks, rain_gauge_sensor_id) values (%s, %s, %s);"
    values = (msg["log_dt"], int(msg["msg"]["TI"]), int(msg["rain_props"]["sensor_id"]))    
    sql_execute_commit(SQL_INSERT, values)

def insert_gkn_tilt_log(msg):
    try:
        SQL_INSERT = """INSERT IGNORE INTO gkn_tilt_sensor_logs (log_datetime, gkn_tilt_sensor_id,
            angle_a, angle_b, volts_a, volts_b, temp) 
            values (%s, %s, %s, %s, %s, %s, %s);
        """
        values = (msg["log_dt"], float(msg["gkn_tilt_props"]["sensor_id"]),
            float(msg["msg"]["AA"]), float(msg["msg"]["AB"]), float(msg["msg"]["VA"]), float(msg["msg"]["VB"]), float(msg["msg"]["TP"])
            )
    except KeyError:
        SQL_INSERT = """INSERT IGNORE INTO gkn_tilt_sensor_logs (log_datetime, gkn_tilt_sensor_id,
            angle_a, angle_b, volts_a, volts_b) 
            values (%s, %s, %s, %s, %s, %s);
        """
        values = (msg["log_dt"], float(msg["gkn_tilt_props"]["sensor_id"]),
            float(msg["msg"]["AA"]), float(msg["msg"]["AB"]), float(msg["msg"]["VA"]), float(msg["msg"]["VB"])
            )
    sql_execute_commit(SQL_INSERT, values)
...
def parse_message(msg):
    ...
    elif msg_dict["msg_type"]=="gkn_tilt":
        msg_dict["gkn_tilt_props"] = get_logger_props(msg_dict,d_type="gkn_tilt",search_for=msg_dict["msg"]["NI"])
    else:
    ...
    if "gkn_tilt_props" in msg_dict.keys():
        insert_gkn_tilt_log(msg_dict)
...
if __name__ == "__main__":
    ...
    messages = get_unparsed_messages(args.limit)

    log.info("Start parsing {}".format(args.limit))

    for index, row in messages.iterrows():
        try:
            # print(row.id, row.message)
            row.message = back_comp_msg(row.message)
            log.info(row.message)
            parse_message(row.message)
        # except:
        #     record_unparsed_message(row.id)
        #     log.error('')
        except Exception as e:
        # except KeyboardInterrupt:
            print("parsing Error", e)
            record_unparsed_message(row.id)
            log.error(str(row.id)+str(traceback.format_exc()))

        try:
            update_message_parsed(row.id)
        except Exception as e:
