...
class GsmModem:
    ...
    def at_cmd(self, cmd = "", expected_reply = 'OK'):
        ...
        try:
            ...
            while (a.find(expected_reply) < 0 and a.find('ERROR') < 0 and 
                ...
                a += self.gsm.read(self.gsm.inWaiting()).decode()
    ...
    def get_all_sms(self):
        ...
        for msg in allmsgs:
            ...
            # txtdatetimeStr = smsdata['date'] + td(hours=8)
                txtdatetimeStr = smsdata['time'].strftime('%Y-%m-%d %H:%M:%S')
            except KeyError:
                print(smsdata)
                txtdatetimeStr = datetime.datetime.today().strftime('%Y-%m-%d %H:%M:%S')
    ...
    #     # preamble = "AT+CMGS=%d" % (len(pdu))
    #     preamble = "AT+CMGS=\"{0}\"".format(number)
    #     print("Preamble:",preamble)

    #     self.gsm.write((preamble+"\r").encode())
    #     now = time.time()
    ...
    #     a = ''
    #     # now = time.time()
    #     # self.gsm.write((pdu.pdu+chr(26)).encode())
    #     # self.gsm.write((pdu+chr(27)).encode())
    #     # print("Data:",pdu.data.decode())
    #     self.gsm.write(msg.encode())
    #     self.gsm.write(chr(26).encode())
    #     now = time.time()
    #     while (a.find('CMGS') < 0 and a.find("ERROR") < 0 and 
    #         time.time() < now + self.SENDING_REPLY_TIMEOUT):
    #             # a += self.gsm.read(self.gsm.inWaiting())
    #             a += self.gsm.read(self.gsm.inWaiting()).decode()
    #             time.sleep(self.WAIT_FOR_BYTES_DELAY)
    #             print (':',end="")
    ...
    def send_msg(self, msg, number, simulate=False):
        ...
        for pdu in pdulist:
            ...
            # preamble = "AT+CMGS=%d" % (pdu.length)
            # preamble = "AT+CMGS=%d" % (len(pdu))
            preamble = "AT+CMGS=%d" % (pdu.tpduLength)
            ...
            # self.gsm.write((pdu.pdu+chr(26)).encode())
            # self.gsm.write((pdu+chr(27)).encode())
            # print("Data:",str(codecs.encode(pdu.data, 'hex_codec'), 'ascii').upper())
            line = str(codecs.encode(pdu.data, 'hex_codec'), 'ascii').upper()

            # print("Data:",line)
            # self.gsm.write(bytearray(pdu.data))
            # for i in pdu.data:
            #     self.gsm.write(i)
            self.gsm.write(bytearray(line,"utf-8"))
            self.gsm.write(chr(26).encode())
            now = time.time()
